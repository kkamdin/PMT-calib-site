...
function init() {
    ...
    $.getJSON('js/pca_constants.json', function(data) { pca = data });
...
function populateRunTable(pca_runs) {
    $.each(pca_runs, function(idx, thisRun) {
        var row = $('<tr/>', {
        ...
        var gainFitCell = $('<td/>', {
            "class": thisRun['gainfit_pass'] ? "pass" : "fail"
        });
        gainFitCell.append("<em>" +
                          (thisRun['gainfit_pass'] ? 'Pass' : 'Fail') +
                          "</em>");

        var timeWalkCell = $('<td/>', {
            "class": thisRun['timewalk_pass'] ? "pass" : "fail"
        });
        timeWalkCell.append("<em>" +
                            (thisRun['timewalk_pass'] ? 'Pass' : 'Fail') +
                            "</em>");
...
function createDetailsPanel(targetRun) {
    ...
    $.each(pca.flags, function(mode, flags) {
        var category = $("<a/>", {
            href: "#",
...
function drawFilterOptions(rowSelector) {
    ...
    $.each(pca.flags[runMode], function(idx, flag) {
        if (flag['name'] == "spare"
            || flag['name'] == "TW spare"
